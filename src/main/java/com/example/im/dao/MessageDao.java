...
public interface MessageDao extends JpaRepository<Message, String> {
     ...
     * @return
     */
    @Query(value = "select * from message where is_group = true and receiver_id = ?1 and status = 0 and gmt_create > date_sub(current_timestamp(),INTERVAL 6 MONTH) order by ?#{#pageable}",
            countQuery = "select count(*) from message where is_group = true and receiver_id = ?1 and status = 0 and gmt_create > date_sub(current_timestamp(),INTERVAL 6 MONTH)",
            nativeQuery = true)
     ...
     * @return
     */
    @Query(value = "select * from message where is_group = false and (receiver_id = ?1 && sender_id = ?2 or receiver_id = ?2 && sender_id = ?1) and status = 0 and gmt_create > date_sub(current_timestamp(),INTERVAL 6 MONTH) order by ?#{#pageable}",
            countQuery = "select count(*) from message where is_group = false and (receiver_id = ?1 && sender_id = ?2 or receiver_id = ?2 && sender_id = ?1) and status = 0 and gmt_create > date_sub(current_timestamp(),INTERVAL 6 MONTH)",
            nativeQuery = true)
    List<Message> findByFriend(String userId, String friendId, Pageable pageable);
    @Query(value = "select * from message where (is_group = false and (receiver_id = ?1 or sender_id = ?1) or is_group = true and sender_id = ?1) and status = 0 and content like %?2% and gmt_create > date_sub(current_timestamp(),INTERVAL 6 MONTH)", nativeQuery = true)
